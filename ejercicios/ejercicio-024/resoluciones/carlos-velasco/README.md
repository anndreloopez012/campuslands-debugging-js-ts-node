# Ejercicio 024 - Ciencia ficción

## Error encontrado

Se encontraron dos errores en la implementación de `space-missions.ts`.
...
```bash
npx vitest run ejercicios/ejercicio-024/tests/space-missions.test.ts
```
...
Los tests definidos en `space-missions.test.ts` son:
...
```text
ejercicios/ejercicio-024/resoluciones/carlos-velasco/
├── space-missions.ts
└── README.md
```
...
Aunque el contexto general del ejercicio indica como objetivo "Clasificar misiones por riesgo", el archivo base y los tests proporcionados implementan y verifican una funcionalidad de registros, promedio de activos y obtención del mayor puntaje. Por ello, la solución se basa en el contrato real y verificable definido por `space-missions.ts` y `space-missions.test.ts`, sin inventar una funcionalidad diferente a la proporcionada.

## Conclusión

Se encontraron dos bugs en `space-missions.ts`.
...
Los tests originales no fueron modificados y los dos casos definidos por `space-missions.test.ts` deben pasar correctamente.
