...
El `README.md` base describe el objetivo como "no exponer stack y usar status correcto"
(tema manejo centralizado de errores, dificultad "Dificil"), pero el archivo real
`codigo/app.js` no contiene ningun middleware de manejo de errores ni expone ningun
stack trace: solo tiene `GET /health`, `GET /items/:id` y `POST /items`, con los mismos
bugs de comparacion de id y status HTTP que los ejercicios 035 a 045. Se corrigio el
codigo tal como existe realmente, sin agregar manejo centralizado de errores que el test
no pide.
