...
El `README.md` base describe el objetivo como "corregir contrato entre ruta y servicio"
(tema capas controller/service, dificultad "Dificil"), pero el archivo real
`codigo/app.js` no separa ninguna capa de controlador ni servicio: toda la logica esta
directamente en las rutas de `GET /health`, `GET /items/:id` y `POST /items`, con los
mismos bugs de comparacion de id y status HTTP que los ejercicios 035 a 046. Se corrigio
el codigo tal como existe realmente, sin agregar capas que el test no pide.
