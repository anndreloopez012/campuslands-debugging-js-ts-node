...
El `README.md` base describe el objetivo como "esperar promesas antes de responder"
(tema servicios asincronicos, dificultad "Dificil"), pero el archivo real
`codigo/app.js` no contiene ninguna promesa, `async`/`await` sin resolver, ni operacion
asincrona real: solo tiene `GET /health`, `GET /items/:id` y `POST /items`, con los
mismos bugs de comparacion de id y status HTTP que los ejercicios 035 a 044. Se corrigio
el codigo tal como existe realmente, sin agregar logica asincrona que el test no pide.
