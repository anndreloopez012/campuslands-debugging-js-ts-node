...
Se encontraron dos errores en el archivo `damage-engine.ts`.

Aunque el objetivo indicado en el README es "Resolver dano con armadura y distancia", el código y los tests proporcionados implementan y validan una funcionalidad diferente: calcular el promedio de registros activos y obtener el registro con mayor puntaje.

Por esta razón, la solución se basa en el contrato real definido por `damage-engine.ts` y `damage-engine.test.ts`.
...
```bash
npx vitest run ejercicios/ejercicio-029/tests/damage-engine.test.ts
```
...
```text
ejercicios/ejercicio-029/resoluciones/carlos-velasco/
+-- damage-engine.ts
+-- README.md
```
...
Se corrigieron dos errores en `damage-engine.ts`.
...
La discrepancia entre el objetivo del README, que menciona daño con armadura y distancia, y el comportamiento real definido por el código y los tests queda documentada. La solución sigue el contrato verificable de los tests existentes.