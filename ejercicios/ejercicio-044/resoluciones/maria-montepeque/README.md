...
El `README.md` base describe el objetivo como "responder errores de validacion claros"
(tema validacion con Zod, dificultad "Dificil inicial"), pero el archivo real
`codigo/app.js` no importa ni usa `zod` en ningun momento: solo tiene `GET /health`,
`GET /items/:id` y `POST /items`, con los mismos bugs de comparacion de id y status HTTP
que los ejercicios 035 a 043. Se corrigio el codigo tal como existe realmente, sin
agregar validacion con Zod que el test no pide.
