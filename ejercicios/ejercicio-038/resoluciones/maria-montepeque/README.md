...
El `README.md` base describe el objetivo como "corregir PATCH parcial sin borrar
campos", pero el archivo real `codigo/app.js` no expone ningun endpoint `PATCH`: solo
tiene `GET /health`, `GET /items/:id` y `POST /items`, con los mismos bugs de
comparacion de id y status HTTP que los ejercicios 035, 036 y 037. Se corrigio el codigo
tal como existe realmente, sin agregar funcionalidad que el test no pide.
