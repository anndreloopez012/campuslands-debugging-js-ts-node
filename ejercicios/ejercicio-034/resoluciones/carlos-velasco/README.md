...
Se encontraron dos errores en la implementación de `tourism.ts`.
...
La función filtraba correctamente los registros que debían participar en el promedio:
...
```bash
npx vitest run ejercicios/ejercicio-034/tests/tourism.test.ts
```
...
Los tests definidos en `tourism.test.ts` son:
...
```text
ejercicios/ejercicio-034/resoluciones/carlos-velasco/
├── tourism.ts
└── README.md
```
...
Aunque el contexto general del ejercicio indica como objetivo "Filtrar paquetes por presupuesto y temporada", el archivo base y los tests proporcionados implementan y verifican una funcionalidad de registros, promedio de activos y obtención del mayor puntaje. Por ello, la solución se basa en el contrato real y verificable definido por `tourism.ts` y `tourism.test.ts`, sin inventar una funcionalidad diferente a la proporcionada.

## Conclusión

Se encontraron dos bugs en `tourism.ts`.
...
Los tests originales no fueron modificados y los dos casos definidos por `tourism.test.ts` deben pasar correctamente.