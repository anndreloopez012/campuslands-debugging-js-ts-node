...
Se encontraron dos errores en la implementación de `rpg-character.ts`.
...
La función correctamente filtraba los registros que debían participar en el promedio:

```typescript
const activos = registros.filter((registro) => registro.activo !== false);
```

Sin embargo, después utilizaba:
...
El registro `beta` está inactivo y, por lo tanto, no debe participar en el cálculo.
...
La causa raíz fue utilizar diferentes conjuntos de datos para el numerador y el denominador del promedio.
...
En `sort()`, un resultado negativo indica que `a` debe aparecer antes que `b`.
...
coloca primero los valores menores.
...
```bash
npx vitest run ejercicios/ejercicio-019/tests/rpg-character.test.ts
```
...
Los tests definidos exactamente en `rpg-character.test.ts` son:
...
```text
ejercicios/ejercicio-019/resoluciones/carlos-velasco/
├── rpg-character.ts
└── README.md
```
...
Aunque el contexto general menciona personajes RPG y el objetivo habla de corregir tipos y calcular poder total, el test proporcionado define el comportamiento real que debe cumplir este ejercicio: calcular un promedio de registros activos y obtener el registro con mayor puntaje. Por ello, la solución prioriza el contrato verificable del test.

## Conclusión

Se encontraron dos bugs en `rpg-character.ts`.
...
Los tests originales no fueron modificados y los dos casos definidos por `rpg-character.test.ts` deben pasar correctamente.