...
El `README.md` base describe el objetivo como "evitar carrera al reservar cupos" (tema
concurrencia simulada, dificultad "Avanzado basico"), pero el archivo real
`codigo/app.js` no contiene ninguna logica de reserva de cupos ni condiciones de
carrera: solo tiene `GET /health`, `GET /items/:id` y `POST /items`, con los mismos bugs
de comparacion de id y status HTTP que los ejercicios 035 a 048. Se corrigio el codigo
tal como existe realmente, sin agregar control de concurrencia que el test no pide.
