...
El `README.md` base describe el objetivo como "validar token y roles por endpoint" (tema
auth simple, dificultad "Dificil inicial"), pero el archivo real `codigo/app.js` no
contiene ninguna logica de autenticacion ni roles: solo tiene `GET /health`,
`GET /items/:id` y `POST /items`, con los mismos bugs de comparacion de id y status HTTP
que los ejercicios 035 a 041. Se corrigio el codigo tal como existe realmente, sin
agregar auth que el test no pide.
