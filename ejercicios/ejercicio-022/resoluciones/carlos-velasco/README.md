...
Se encontraron dos errores en la implementación de `welding.ts`.
...
En `sort()`, este comparador coloca primero los valores menores.
...
```bash
npx vitest run ejercicios/ejercicio-022/tests/welding.test.ts
```
...
Los tests definidos en `welding.test.ts` son:
...
```text
ejercicios/ejercicio-022/resoluciones/carlos-velasco/
├── welding.ts
└── README.md
```
...
Aunque el contexto general del ejercicio indica como objetivo "Calcular consumo de varilla por cordón", el archivo base y los tests proporcionados implementan y verifican una funcionalidad de registros, promedio de activos y obtención del mayor puntaje. Por ello, la solución se basa en el contrato real y verificable definido por `welding.ts` y `welding.test.ts`, sin inventar una funcionalidad diferente a la proporcionada.

## Conclusión

Se encontraron dos bugs en `welding.ts`.
...
Los tests originales no fueron modificados y los dos casos definidos por `welding.test.ts` deben pasar correctamente.
