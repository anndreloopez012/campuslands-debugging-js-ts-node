...
El `README.md` base describe el objetivo como "invalidar cache despues de actualizar"
(tema cache simple, dificultad "Dificil"), pero el archivo real `codigo/app.js` no
contiene ninguna logica de cache: solo tiene `GET /health`, `GET /items/:id` y
`POST /items`, con los mismos bugs de comparacion de id y status HTTP que los ejercicios
035 a 047. Se corrigio el codigo tal como existe realmente, sin agregar cache que el
test no pide.
