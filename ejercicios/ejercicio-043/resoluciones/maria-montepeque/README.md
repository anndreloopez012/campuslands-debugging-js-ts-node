...
El `README.md` base describe el objetivo como "corregir limit/offset y metadatos" (tema
paginacion, dificultad "Dificil inicial"), pero el archivo real `codigo/app.js` no
contiene ningun parametro de paginacion (`limit`/`offset`) ni metadatos: solo tiene
`GET /health`, `GET /items/:id` y `POST /items`, con los mismos bugs de comparacion de id
y status HTTP que los ejercicios 035 a 042. Se corrigio el codigo tal como existe
realmente, sin agregar paginacion que el test no pide.
