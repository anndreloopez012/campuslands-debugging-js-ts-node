...
El `README.md` base describe el objetivo como "separar errores 404 y 422", pero el
archivo real `codigo/app.js` no contiene ninguna validacion que devuelva `422`: solo
tiene `GET /health`, `GET /items/:id` y `POST /items`, con los mismos bugs de comparacion
de id y status HTTP que los ejercicios 035 a 039. Se corrigio el codigo tal como existe
realmente, sin agregar validaciones que el test no pide.
