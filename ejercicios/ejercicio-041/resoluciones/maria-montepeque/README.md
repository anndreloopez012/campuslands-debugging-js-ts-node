...
El `README.md` base describe el objetivo como "propagar requestId en errores" (tema
middleware de request id, dificultad "Dificil inicial"), pero el archivo real
`codigo/app.js` no contiene ningun middleware ni requestId: solo tiene `GET /health`,
`GET /items/:id` y `POST /items`, con los mismos bugs de comparacion de id y status HTTP
que los ejercicios 035 a 040. Se corrigio el codigo tal como existe realmente, sin
agregar middleware que el test no pide.
