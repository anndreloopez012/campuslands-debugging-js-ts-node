...
Se encontraron dos errores en la implementación de `skydiving.ts`.
...
```bash
npx vitest run ejercicios/ejercicio-032/tests/skydiving.test.ts
```
...
Los tests definidos en `skydiving.test.ts` son:
...
```text
ejercicios/ejercicio-032/resoluciones/carlos-velasco/
├── skydiving.ts
└── README.md
```
...
Aunque el contexto general del ejercicio indica como objetivo "Validar checklist de salto", el archivo base y los tests proporcionados implementan y verifican una funcionalidad de registros, promedio de activos y obtención del mayor puntaje. Por ello, la solución se basa en el contrato real y verificable definido por `skydiving.ts` y `skydiving.test.ts`, sin inventar una funcionalidad diferente a la proporcionada.

## Conclusión

Se encontraron dos bugs en `skydiving.ts`.
...
Los tests originales no fueron modificados y los dos casos definidos por `skydiving.test.ts` deben pasar correctamente.
