...
Se encontraron dos errores en la implementación de `luxury-cars.ts`.
...
```bash
npx vitest run ejercicios/ejercicio-031/tests/luxury-cars.test.ts
```
...
Los tests definidos en `luxury-cars.test.ts` son:
...
```text
ejercicios/ejercicio-031/resoluciones/carlos-velasco/
├── luxury-cars.ts
└── README.md
```
...
Aunque el contexto general del ejercicio indica como objetivo "Ordenar catálogo por potencia/precio", el archivo base y los tests proporcionados implementan y verifican una funcionalidad de registros, promedio de activos y obtención del mayor puntaje. Por ello, la solución se basa en el contrato real y verificable definido por `luxury-cars.ts` y `luxury-cars.test.ts`, sin inventar una funcionalidad diferente a la proporcionada.

## Conclusión

Se encontraron dos bugs en `luxury-cars.ts`.
...
Los tests originales no fueron modificados y los dos casos definidos por `luxury-cars.test.ts` deben pasar correctamente.
