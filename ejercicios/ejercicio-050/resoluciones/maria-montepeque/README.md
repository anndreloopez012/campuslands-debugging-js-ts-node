...
El `README.md` base describe el objetivo como "resolver multiples fallas en flujo CRUD"
(tema depuracion integral, dificultad "Avanzado basico"), pero el archivo real
`codigo/app.js` no expone un CRUD completo (no hay `PUT`, `PATCH` ni `DELETE`): solo
tiene `GET /health`, `GET /items/:id` y `POST /items`, con los mismos bugs de
comparacion de id y status HTTP que los ejercicios 035 a 049. Se corrigio el codigo tal
como existe realmente, sin agregar endpoints que el test no pide.
